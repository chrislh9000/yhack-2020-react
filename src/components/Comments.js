  ...
  render() {
    ...
    return (
      ...
      {this.props.pins.map((item, i) => (
        ...
        <Form style={{paddingBottom: '10px'}} onSubmit={(e) => this.handleSubmit(e, item, i)} onKeyDown={(e) => this.initSubmit(e, item, i)}>
        <Form.Label style = {{color: "white"}}>Note: </Form.Label>
        {
            this.state.editedPin === i ?
            <Form.Control size="sm" onClick={(e) => this.handleNoteChange(e, item, i)} onChange={this.handleChange} value={this.state.value} placeholder={item.note} as="textarea" rows={2} />
            :
            <Form.Control type="text" size="sm" onClick={(e) => this.handleNoteChange(e, item, i)} onChange={this.handleChange} value={item.note} placeholder={item.note} />
        }
        </Form>
        </div>
      ))}
